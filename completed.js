...
//     { name: "Jay", age: 24 },
//   ];
//   users.forEach(user => console.log(user.age)); // this loop is only used to print the objects

//   let adults = users.filter(user => user.age >=18); // this loop is used to apply some conditions to the object
//   adults.forEach(n => console.log( n.name));

// names = users.map(n => n.name) ; // another examples for printing the name
// console.log(`this are th e name of the object ${names}`);

// sum1 = users.reduce((sum , user)=> sum + user.age , 0);
// console.log(`THE AVARAGE age ${sum1/users.length}`)

  //  THE TASK OF THE OBJECT Create an array of 5 products
     
    // Get total price of all products using reduce().
    // BONUS: Make a new array with names + " is a great product!" using map()
    // Like: ["Shirt is a great product!", "Watch is a great product!"]

//   products=[{
// 'name':'Rayon','price':160 , 'category':'fashion'
//   },{'name':'Cotton','price':150 , 'category':'fashion'

//   },{ 'name':'Chamki','price':190 , 'category':'electric'

//   },{'name':'Ball','price':100 , 'category':'toy'

//   }]

// n_arr= products.map(n => (`${n.name} is a great product!` ))


// let price = products.reduce((sum , n) => sum+ n.price,0)
// console.log(n_arr)
...
//  real world challanges
  ...
  // let inventory = [
  //   { name: "Cotton Dupatta", quantity: 10, sold: 4 },
  //   { name: "Rayon Dupatta", quantity: 5, sold: 1 },
  //   { name: "Silk Dupatta", quantity: 20, sold: 10 },
  //   { name: "Fancy Dupatta", quantity: 0, sold: 0 },
  // ];

  // // Print stock left for each product


  // let stk =inventory.filter(n =>n.quantity >0)
  // stk.forEach(n =>console.log(`The ${n.name} is avaiable because the quanilty in store is ${n.quantity} `) )
  // let stk2 =inventory.filter(f =>f.quantity <1)
  // stk2.forEach(f =>console.log(`The ${f.name} is unavaiable because the quanilty in store is ${f.quantity} `) )

  // let p =stk.reduce((sum,n) => sum+n.sold,0)
  // console.log(`total sold item is ${p}`)
//   let inventory = [
//     { name: "Cotton Dupatta", price: 150, quantity: 20, sold: 12, fabric: "cotton" },
//     { name: "Silk Dupatta", price: 250, quantity: 0, sold: 30, fabric: "silk" },
//     { name: "Rayon Dupatta", price: 180, quantity: 10, sold: 5, fabric: "rayon" },
//     { name: "Chiffon Dupatta", price: 130, quantity: 5, sold: 3, fabric: "chiffon" },
//     { name: "Net Dupatta", price: 200, quantity: 0, sold: 0, fabric: "net" }
//   ];

//   let result = inventory.map(n => ({
//     name: n.name,
//     stockLeft: n.quantity - n.sold,
//     status: (n.quantity - n.sold) > 0 ? "Available" : "Out of Stock"
//  }));
 
//   console.log(result);

  // { name: "Cotton Dupatta", stockLeft: 8, status: "Available" },
